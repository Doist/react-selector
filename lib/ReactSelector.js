...
(function() {
  ...
  ReactSelector = React.createClass({
    ...
    _getItems: function(list, filtering) {
      var i, is_active_item, item, item_class_name, item_component, items, j, len, sorted_list, special_items;
      if (filtering == null) {
        filtering = false;
      }
      items = [];
      ...
      sorted_list.sort(this.props.compare);
      for (i = j = 0, len = sorted_list.length; j < len; i = ++j) {
        item = sorted_list[i];
